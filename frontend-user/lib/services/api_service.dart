...
class ApiService {
  ...
  Future<List<CitizenRequestDto>> getCitizenRequests({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return list
            .map((e) => CitizenRequestDto.fromJson(e as Map<String, dynamic>))
            .toList();
      }
      return [];
    } on DioException catch (e) {
      _logger.e('Get requests error: ${e.message}');
      if (e.response?.statusCode == 404) return [];
      throw Exception('Не удалось загрузить обращения: ${e.message}');
    }
  }
  ...
  Future<Map<String, dynamic>> getAiStats() async {
    try {
      ...
      _logger.e('Get AI stats error: ${e.message}');
      return {};
    }
  }
  ...
  Future<List<Map<String, dynamic>>> searchAddress(String query) async {
    try {
      final response = await _dio.get(
        ...
        // User-Agent нельзя установить в браузере, но Nominatim работает и без него
        options: Options(),
      );
      return (response.data as List<dynamic>)
          .cast<Map<String, dynamic>>()
  ...
  // Address autocomplete (for suggestions while typing)
  Future<List<String>> autocompleteAddress(String query) async {
    if (query.trim().length < 3) return [];
    
    try {
      // Добавляем "Новосибирск" для лучших результатов
      final searchQuery = query.contains('Новосибирск') || query.contains('Ново')
          ? query
          : '$query, Новосибирск';
      ...
      // Извлекаем полные адреса из результатов
      return results.map((r) {
        final displayName = r['display_name'] as String? ?? '';
        // Убираем лишние части адреса для краткости
        if (displayName.contains(',')) {
          final parts = displayName.split(',');
          // Берем первые 3-4 части (улица, дом, район, город)
          return parts.take(4).join(', ').trim();
        }
        return displayName;
      }).toList();
    } on DioException catch (e) {
      _logger.e('Autocomplete address error: ${e.message}');
      return [];
    }
  }
  ...
  Future<CitizenRequestDto> submitNewRequest({
    ...
    int? categoryId, // Теперь опционально - нейросеть определит автоматически
    ...
    try {
      final data = <String, dynamic>{
        ...
        if (categoryId != null) 'categoryId': categoryId, // Отправляем только если указано
      };
