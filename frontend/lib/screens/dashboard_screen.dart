...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        _requests = results[0] as List<CitizenRequestDto>;
        _aiStats = results[1] as Map<String, dynamic>;
        _loading = false;
      });
      _buildMarkers();
    } catch (_) {
  ...
  Widget _buildKPIPanel() {
    ...
    final analyzed = _aiStats?['analyzedRequests'] as int? ?? 0;
    final coverage = _aiStats?['analysisCoveragePercent'] as double? ?? 0.0;
